...
interface Props {
  ...
  onReady?: () => void;
  active?: boolean;
  onPoseFrame?: (world: any[], image: any[]) => void;
}

export default function TherapistScene({ objectPath, onReady, active = true, onPoseFrame }: Props) {
  const mountRef = useRef<HTMLDivElement>(null);
  const videoRef = useRef<HTMLVideoElement>(null);
  const [status, setStatus] = useState("Loading…");

  // Keep stable refs so rAF/event callbacks always see the latest values
      ...
      .then((PoseClass) => {
        ...
        pose.onResults((results: any) => {
          if (results.poseLandmarks && results.poseWorldLandmarks) {
            ...
            // Bubble current frame's pose data up to SessionPage for comparison
            onPoseFrameRef.current?.(results.poseWorldLandmarks, results.poseLandmarks);
            if (!firstPoseDone) {
              firstPoseDone = true;
              maybeFireReady();
            }
